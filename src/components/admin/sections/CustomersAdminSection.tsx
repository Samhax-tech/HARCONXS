...
import { CustomerRecord, CustomerAddressItem, ProductReview } from '../../../types';
...
export const CustomersAdminSection: React.FC<CustomersAdminSectionProps> = ({
  ...
  const handleDeleteReview = async (id: string, author: string) => {
    const confirm = window.confirm(`Permanently remove review by "${author}"?`);
    if (!confirm) return;
    try {
      await enforceServerSidePermission('reviews:delete', 'review', id);
      const success = await deleteProductReview(id);
      if (success) {
        showToast(`Review by "${author}" deleted.`);
      }
    } catch (err: any) {
      showToast(err.message || 'Permission Denied: Only authorized managers can delete reviews.');
    }
  };
  ...
  const handleCreateReview = async (e: React.FormEvent) => {
    ...
    if (!reviewForm.productId || !reviewForm.userName || !reviewForm.comment) {
      showToast('Please select a product and provide reviewer name and review text.');
      return;
    }

    try {
      await enforceServerSidePermission('reviews:create', 'review', 'new');
      const res = await addProductReview({
        productId: reviewForm.productId,
        userName: reviewForm.userName,
        userEmail: reviewForm.userEmail || undefined,
        rating: Number(reviewForm.rating),
        title: reviewForm.title || 'Verified Client Experience',
        comment: reviewForm.comment,
        verified: reviewForm.verifiedPurchase,
        verifiedPurchase: reviewForm.verifiedPurchase,
        status: 'approved'
      });

      if (res.success) {
        showToast('New client review created and published.');
      ...
      showToast(err.message || 'Permission Denied: Unable to create review.');
    }
  };
    ...
    <div id="customers-admin-section" className="space-y-6">
      ...
      {subSection === 'reviews' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              <p className="text-xs text-zinc-400">
                Moderate, add, and curate verified client reviews and testimonials displayed across store product pages.
              </p>
            </div>
            <button
              id="add-admin-review-btn"
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <select
                ...
                className="px-3 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-300 text-sm focus:outline-none focus:border-amber-400 cursor-pointer"
              >
                <option value="all">All Reviews</option>
                <option value="approved">Approved & Published</option>
                <option value="pending">Pending Moderation</option>
                <option value="5_star">5-Star Ratings</option>
              </select>
            </div>
          </div>
              ...
              <table className="w-full text-left text-sm text-zinc-300">
                ...
                <tbody className="divide-y divide-zinc-800">
                  {reviews
                    ...
                    .filter(rev => {
                      if (reviewFilter === 'approved') return rev.status === 'approved' || (rev as any).isApproved;
                      if (reviewFilter === 'pending') return rev.status === 'pending' || !(rev as any).isApproved;
                      if (reviewFilter === '5_star') return rev.rating === 5;
                      return true;
                    })
                    .map(rev => {
                      const isApproved = rev.status === 'approved' || (rev as any).isApproved;
                      ...
                      return (
                        <tr key={rev.id} className="hover:bg-zinc-800/40 transition-colors">
                          <td className="py-3 px-4">
                            ...
                            {rev.date && <div className="text-[10px] text-zinc-500 font-mono mt-0.5">{rev.date}</div>}
                          </td>
                          <td className="py-3 px-4 text-xs font-mono text-zinc-300">
                            {prod ? prod.name : (rev.productId || 'Atelier Item')}
                          </td>
                          <td className="py-3 px-4">
                          ...
                          <td className="py-3 px-4 text-xs max-w-xs">
                            ...
                            <div className="text-zinc-400 line-clamp-2">{rev.comment || rev.review}</div>
                          </td>
                          <td className="py-3 px-4">
                            ...
                            <div className="flex items-center justify-end gap-2">
                              {!isApproved ? (
                                <button
                                  ...
                                  className="px-2.5 py-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-xs font-bold border border-emerald-500/30 cursor-pointer"
                                ...
                                <button
                                  ...
                                  className="px-2.5 py-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-zinc-200 text-xs border border-zinc-700 cursor-pointer"
      ...
      {isAddReviewModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-lg p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
              <div>
                <h3 className="text-xl font-serif font-bold text-zinc-100 flex items-center gap-2">
                  <Star className="w-5 h-5 text-amber-400 fill-amber-400" />
                  Add Patron Testimonial
                </h3>
                <p className="text-xs text-zinc-400 mt-1">
                  Create a verified customer review linked to any store catalog product.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleCreateReview} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-300 mb-1">Catalog Product *</label>
                <select
                  required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Reviewer Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Reviewer Email (Optional)</label>
                  <input
                    type="email"
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-amber-400 ml-2">{reviewForm.rating} of 5 Stars</span>
                </div>
              </div>

              <div>
                <label className="block text-xs font-medium text-zinc-300 mb-1">Review Headline Title</label>
                <input
                  type="text"
                  placeholder="e.g. Beyond exceptional craft and heirloom finish"
              ...
              <div>
                ...
                <textarea
                  rows={4}
                  required
                  placeholder="Share details of the bespoke unboxing, custom engraving, and craftsmanship experience..."
